...
qmax_HFT = 10*qmax
call_intervals = [1,100]
delay = 100


for jump in range(2):
    call_interval=call_intervals[jump]
    ...
    for _, b, s, p, bq, sq in trade_records:
        ...
        if seller.is_fast:
            ...
            slow_sum += ss
    # Print
    mar_type="CALL" if jump>0 else "CDA"
    
    print(f"Simulation complete: market={mar_type}, batch_interval={call_interval}")
    print(f"Efficiency (social/optimum): {eff*100:.4f}%" if eff else "No eff data")