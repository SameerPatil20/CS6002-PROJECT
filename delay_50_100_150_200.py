...
qmax_HFT = 10*qmax    # HFT inventory bound
call_interval = 1     # initial call interval
delays = [50,100,150,200] # delays to test
...
for delay in delays:
    ...
    for i in range(1,52, 8):
        dp.append(i)
    for i in range(76,502,25):
        dp.append(i)
    ...
    for jump in dp:
        ...
        # High-frequency trader -> is_fast=True
    ...
    plt.plot(interval, effi, linewidth=1, marker='o', label=f'Delay {delay}')
    max_index = effi.index(max(effi))
    x_at_max_y = interval[max_index]

    print(f"Maximum Efficiency at delay {delay} is : {max(effi)}")
    print("Efficiency of CDA: ", effi[0])

plt.xlabel('Batch Interval')
plt.ylabel('Efficiency')
plt.title('Efficiency vs Batch Interval')
plt.grid(True)
plt.legend()
